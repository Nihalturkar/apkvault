...
export default function AppForm({ initialData, onSubmit, submitLabel = "Save App" }) {
  ...
  function uploadFile(file, slug, type, onProgress) {
  ...
  async function handleIconUpload(e) {
    ...
    try {
      const result = await uploadFile(file, slug, "icon");
  ...
  async function handleApkUpload(e) {
    ...
    setApkProgress(0);
    try {
      const result = await uploadFile(file, slug, "apk", (pct) => setApkProgress(pct));
      handleChange("apkUrl", result.url);
      handleChange("sizeMb", parseFloat((result.size / (1024 * 1024)).toFixed(1)));
    } catch (err) {
  ...
  async function handleScreenshotUpload(e) {
    ...
    try {
      for (const file of files) {
        ...
        const result = await uploadFile(file, slug, "screenshot");
